...
% value.
% 
% ZOOM (default 'auto') specifies the number of matrix samples per
% screen pixel.  It will be rounded to an integer, or 1 divided by an
% integer.  A value of 'same' or 'auto' causes the zoom value to be
% chosen automatically to fit the image into the current axes.  A
% value of 'full' fills the axis region (leaving no room for labels).
% See pixelAxes.m.
% 
...
% (size) of the matrix are printed below the image.  If label is a string, 
...
if isreal(im)
  ...
  if (any(size(im)==1))
    ...
    subplot(1,2,1);
    hh = image(real(d_im));
    axis('off'); pixelAxes(size(d_im),zoom);
