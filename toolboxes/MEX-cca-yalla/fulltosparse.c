		 ...
		 )
{
    ...
    pr = mxGetPr(prhs[0]);/* pointer to the real data */
    pi = mxGetPi(prhs[0]);;/* pointer to the imaginary data */
    cmplx = (pi==NULL ? 0 : 1); /* data complex? */
    ...
    sr  = mxGetPr(plhs[0]); /* pointer to real data */
    si  = mxGetPi(plhs[0]); /* pointer to imaginary data */
    irs = mxGetIr(plhs[0]); /* pointer to row info */
    jcs = mxGetJc(plhs[0]); /* pointer to col info */
