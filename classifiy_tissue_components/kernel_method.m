...
component_list = {'carcinoma','fat','lymphocyte','vessel','stroma','duct','white space'};
...
for i = 1: num_components
    ...
    parfor j = 1:numFiles
        ...
        features = load(fullfile(features_dir,fileName));
        features_component{j} = features.data(1:end);
    end
...
cmp_ind = [5,4];
all_features_matrix = cat(1,all_features{cmp_ind});
labels = cat(1,all_labels{cmp_ind});
fileNames = cat(1,all_filenames{cmp_ind});
% omit the regions whose area < 5000
ind_area = all_features_matrix(:,end) > 5000;
all_features_matrix = all_features_matrix(ind_area,1:end-1);
