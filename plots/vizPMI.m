function [] = vizPMI(p,I,opts)
    ...
    figure; 
    %%
    subplot(121); [~,ch] = contourf(xx,yy,(((log(w1)).^1)),20); xlabel('Luminance A'); ylabel('Luminance B'); title('log P(A,B)'); axis('image'); colorbar; set(ch,'edgecolor','k');
        
    %%
    subplot(122); [~,ch] = contourf(xx,yy,(((log(w2)).^1)),20); xlabel('Luminance A'); ylabel('Luminance B'); title('PMI_1(A,B)'); axis('image'); colorbar; set(ch,'edgecolor','k');
end