...
RESULTS_DIR = fullfile(DATA_DIR,'evaluation_results','eval_hue_512_512_sig3'); 
...
%         tmp = load(fullfile(RESULTS_DIR,[im_name '.mat']));
%         E_ucm = tmp.data;
%         segmented_image = ucm2colorsegs(E_ucm,I,thresh);
%         imwrite(uint8(segmented_image),fullfile(RESULTS_DIR,[im_name '_segmentedImage.tif']),'Resolution',300);         
%         end
...
for i=1:length(img_list)
        [~,im_name,~] = fileparts(img_list{i});
        filenames = {fullfile(RESULTS_DIR,'image1024',[im_name '_512_512.tif']),...
            fullfile(RESULTS_DIR,'segmented_images',[im_name '_segmentedImage.tif'])};
        ...
        imwrite(MyMontage.cdata,fullfile(RESULTS_DIR,'montageImageSegment',[im_name '_montage.tif']),'Resolution',300);
