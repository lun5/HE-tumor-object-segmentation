...
method_names = {'opp color-PMI','opp color-PJoint, only hue',...
    'gPb-OWT-UCM','Isola lowres accurate','Isola speedy',...
   'EGB','ncuts multiscale','QuadTree','JSEG one scale','JSEG multiscale',...
   'GraphRLM','MeanShift'};
...
%% plot F_boundary
cols = distinguishable_colors(num_dir);
plot_ind = 1:7;
...
symbols = {'*','d','o','^'};
for i = 8:num_dir;
    h(i) = plot(bdry_evalRes{i}(2),bdry_evalRes{i}(3),symbols{i-7},'MarkerFaceColor',cols(i,:),'MarkerEdgeColor',cols(i,:),'MarkerSize',10);  
    hold on;
end
...
symbols = {'h','<','*','d','o','^'};
for i = [6,7,10,11];
    h(i) = plot(Fop_evalRes{i}(2),Fop_evalRes{i}(3),symbols{i-5},'MarkerFaceColor',cols(i,:),'MarkerEdgeColor',cols(i,:),'MarkerSize',10);  
    hold on;
end
legend(h, method_names,'Location','northeastoutside');
hold off;
